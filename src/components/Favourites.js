...
function Favourites() {
  ...
  let si = (curPage - 1) * rows  //start idx
  let ei = Number(si) + Number(rows)  //ending idx

  filteredMovies = filteredMovies.slice(si, ei);  //start aur end kaha hota hai wo block uthake ke dikha do
    ...
    <div className='mt-4 px-2 flex justify-center flex-wrap space-x-2'>
      {
        genres.map((genre) =>
          <button className={
            ...
            onClick={() => {  //this is for buttons jispe click kara wo blue ho jaye
              setCurPage(1)
              setCurGenre(genre)
            }
            }
